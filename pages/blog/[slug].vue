<template>
  <div class="bg-[#EDEDED]">
    <Bnjnav></Bnjnav>
    <SubHeader :title="blog?.title" sub-title=""> </SubHeader>

    <div class="lg:p-10 max-w-5xl m-auto p-4">
      <NuxtImg
        v-if="blog && blog.imgSrc"
        :src="blog.imgSrc"
        width="700"
        height="400"
        format="webp"
        ...
        quality="70"
        class="object-cover m-auto w-full h-60 md:h-72 lg:h-96"
        alt="blog image"
      >
      </NuxtImg>
      ...
      <div
        v-if="blog"
        v-html="blog.body"
        class="bg-[#fff] text-bodyCopy p-6"
      ></div>
    </div>
  </div>
</template>
...
const { data: blog } = await useFetch<Blog>(
  `https://rrapibeta.azurewebsites.net/api/public/posts/Getposts/${appConfig.licenseKey}/${route.params.slug}`
);
